...
| Invariant | Enforced by | Lands in |
|---|---|---|
| **Every column is `NOT NULL` unless it is named nullable, with a reason** | NULL gets past every other constraint on a column: **a `CHECK` passes on NULL, a `UNIQUE` admits any number of NULLs, a foreign key skips a row with a NULL column, and a `DEFAULT` applies only when the column is omitted** — an explicit NULL is stored. So `NOT NULL` is the rule and nullability is the exception: the issue that adds a nullable column names it and says why, and **a schema test holds that list and fails on any other nullable column** | #2, #11 |
| **A row's identity and owner never change** — its `id`, `organization_id`, `actor_id`, `actors.type`, `invites.kind`, any `token_hash` | **`UPDATE` is granted on named columns, never on a table**, and none of these is named. A different owner or kind is a different row | #2, #11 |
...
| **Every audit record names the actor who did it** | `audit_events.actor_id NOT NULL`, referencing `actors`. The system is an actor type, so "the system did it" needs no NULL and no special column | #2 |
