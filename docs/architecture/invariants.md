...
| **Authorization asks for a capability, never a role name** | `db::can(pool, actor, organization, Capability::...)` reads active membership; `OrganizationRole::holds` in `domain` is the only role-to-capability table | in place |
