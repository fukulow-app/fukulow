...
Identity and membership invariants are implemented below. Channels and messages
arrive with #11, and row security with #10.
...
|---|---|---|
| Every `id` is a UUIDv7 **generated by the application**, not the database | Contract; no `DEFAULT` on `id` columns | #11 |
...
| **Every timestamp is `timestamptz`**, stored in UTC | Column type; a test fails on any `timestamp` column. A `timestamp` column is a full migration to fix | #11 |
