...
async fn change_member_role_cannot_target_another_organization() -> Result {
    ...
    let state: (String, String) = sqlx::query_as("SELECT role, status FROM organization_members WHERE organization_id = $1 AND actor_id = $2").bind(b.0).bind(member.0).fetch_one(&db.app).await?;
    assert!(matches!(
        db::change_member_role(&db.app, owner, a, member, OrganizationRole::Admin).await,
        Err(db::ChangeMemberRoleError::NotFound)
    ));
    let after: (String, String) = sqlx::query_as("SELECT role, status FROM organization_members WHERE organization_id = $1 AND actor_id = $2").bind(b.0).bind(member.0).fetch_one(&db.app).await?;
    assert_eq!(state, after);
    let role: String = sqlx::query_scalar("SELECT role FROM team_members WHERE organization_id = $1 AND team_id = $2 AND actor_id = $3").bind(b.0).bind(team.0).bind(member.0).fetch_one(&db.app).await?;
